...
| Параметр | Значение |
|---------|----------|
| ФИО студента | Загреков Назар Петроч |
| Группа | ДБИ 203-рсобф |
| Номер варианта | 8 |
| Предметная область | Учёт клиентов организации |
| Краткое назначение системы | Информации о клиентах организации |
...
|------------------|------------------|
| Пользователь | Может добавлять клиентов, смотреть список, искать нужного |
| Администратор / Оператор | Всё то же самое, плюс может удалять клиентов и править данные |
...
|--------|----------|-------------------|
| Сущность 1 | Человек или компания, с которой работает организация | ФИО, телефон, email, организация |
| Сущность 2 | Компания-партнёр, к которой привязан клиент | Название |
...
|--|---------|------------------|
| 1 | Добавить клиента | Пользователь заполняет поля и сохраняет |
| 2 | Посмотреть всех клиентов | На странице отображается таблица со всеми записями |
| 3 | Найти клиента | Вводишь фамилию в поиск, список сразу фильтруется |
...
| ------------------------------ | -------- |
| Проектирование структуры сайта |Придумал, что будет три блока: добавление, список, поиск|
| Организация файловой структуры |Создал папку проекта и пять файлов: index.html, style.css, script.js, README.md, passport.md|
| Базовая HTML-разметка          |Написал заголовки и секции, подключил стили и скрипт|
| Подключение ресурсов           |Через link и script в index.html|
| Архитектурное разделение слоёв |HTML отвечает за разметку, CSS за внешний вид, JS за поведение|
